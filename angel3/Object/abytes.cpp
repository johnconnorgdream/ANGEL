...
object_bytes initbytes(int len)
{
	...
	else
	{
		...
		bs = (char *)angel_alloc_page((object)res,alloc);
	}
	res->bytes = bs;
	res->type = BYTES;
	res->len  = len;
...
void resizebytes(object_bytes ob)
{
	...
	char *addr = ob->bytes;
	memset(newaddr,0,ob->alloc_size*sizeof(char));
	memcpy(newaddr,addr,oldalloc*sizeof(char));
...
int appendbytes(object_bytes b,object_bytes b1)
{
	if(b->alloc_size < b->len + b1->len)
		resizebytes(b);
	memcpy(b->bytes,b1->bytes,b1->len);
	b->len += b1->len;
	return 1;
}
object_bytes concatbytes(object_bytes b,object_bytes b1)
{
	...
	memcpy(res->bytes+b->len,b1->bytes,b1->len);
	return res;
}
object_bytes insertbytes(object_bytes b,object_bytes b1,int pos)
{
	...
	int copynum;
	object_bytes res = initbytes(GETBYTES(b)->len+GETBYTES(b1)->len);
	memcpy(res->bytes,b->bytes,pos+1);
	memcpy(res->bytes+pos+1,b1->bytes,b1->len);
	memcpy(res->bytes+pos+1+b1->len,b->bytes+pos+1,b->len-pos-1);
	return res;
}
object_bytes bytesrepeat(object_bytes b,int count)
{
	...
	for(i=0; i<count; i++)
	{
		memcpy(res->bytes+eachlen*i,b->bytes,eachlen);
	}
