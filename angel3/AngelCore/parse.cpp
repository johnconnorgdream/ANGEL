...
tstack ctrl_state,op_stack1,op_stack2;  //控制语句的执行状态。他表示你当前是在哪个分支语句下的
...
#define VAL_UNDIFINED(error,name) \
					do{ \
						if(!error) { \
							char errorinfo[100]; \
...
int priority(token t)  //以后通过扩展这个函数的功能来增加跟多的运算，并确定优先级
{
	...
	case SELFSUB:
		return 19;
	case MAKEFUN:  //未来会升级到反射操作REFLECT
	case DYNAMIC_CALL:
		return 20;
	case INDEX:
		return 21;
...
code predeal(FILE *f)
{
	code c=(code)angel_sys_malloc(sizeof(code)); //建立一个头节点
...
void copytree(token src,token *des)  //完整的赋值表达式树
{
	if(src)
	{
		*des=(token)angel_sys_malloc(sizeof(tokennode));   //因为传入的des肯定指向空
		angel_sys_memcpy(*des,src,sizeof(tokennode));   //再将左右子树置空
...
int islambdaordynamiccall(int pos,int ispreoper)  //2为动态调用函数，1为lambda函数，0代表什么都不是
{
	int next = detectbracket(pos,LBRACKETL);
	if(next == -1)
		return 0;
	if((ISEND(next) || t[next]->id != BBRACKETL) && ispreoper){  //动态调用函数
		t[next-1]->id = END;
		return 2;
	}
...
{
#define FREEOTSTACK do{_CLEAR_STACK(ts); _CLEAR_STACK(os);}while(0);
...
#define ISKEYWORD(id) ((id) < IF || (id) > RET)
	token temp;
	RESET_STACK(op_stack1);
	RESET_STACK(op_stack2);
	tstack os = op_stack1,ts = op_stack2;
...
newbeg:
			...
			if(stackempty(os)  ||  priority(t[*pos]) > priority(gettop(os)) || t[*pos]->id==LBRACKETL || t[*pos]->id==ASSIG && gettop(os)->id==ASSIG)  //多个复合赋值运算需要做一些改变
			{
				if(t[*pos]->id==LBRACKETL)  //可能是lambada函数
				{
					...
					if(lambada_flag == 1)  //肯定是函数定义了
					{
						DEALOPER;
						goto fundef;
					}
					else if(lambada_flag == 2) //动态调用
					{
...
#define ISPARSEASLIB (!global_class_env && !f && islib == 1)
	token root=(token)angel_sys_malloc(sizeof(tokennode));  //总的语法根节点
...
int parse_angel(token root)
{
	...
	ctrl_state = (tstack)angel_sys_calloc(1,sizeof(tstacknode));  //这个控制流栈也可以为未来的变量的作用范围的确定服务
	ctrl_state->top = -1;
